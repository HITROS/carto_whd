...
proto::ImuData ToProto(const ImuData& imu_data) {
  ...
  *proto.mutable_linear_acceleration() =
      transform::ToProto(imu_data.linear_acceleration);  //设置imu数据的格式
  *proto.mutable_angular_velocity() =                    //按照transform定义的toproto要求
      transform::ToProto(imu_data.angular_velocity);
  return proto;
}
/*
功能：将imu传递的数据分成 timestamp \ linear_acceleration \ angular_velocity
返回：返回区分后的程序
*/
